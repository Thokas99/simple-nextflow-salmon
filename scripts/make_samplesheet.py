...
FASTQ_RE = re.compile(
    r"^(?P<sample>.+?)[_.-]R?(?P<read>[12])(?:[_.-]001)?(?P<ext>\.f(?:ast)?q(?:\.gz)?)$",
    re.IGNORECASE,
)
...
def find_fastqs(fastq_dir: Path) -> list[Path]:
    return sorted(
        ...
        if p.is_file() and FASTQ_RE.match(p.name)
    )
...
def build_rows(fastqs: list[Path]) -> list[dict[str, str]]:
    ...
    if missing:
        raise SystemExit("Missing pair for sample(s): " + ", ".join(sorted(missing)))
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create sample,fastq_1,fastq_2 CSV from paired FASTQ files."
    )
